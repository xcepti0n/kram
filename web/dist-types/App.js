...
export function App() {
    ...
    useEffect(() => {
        ...
        try {
            localStorage.setItem('tasktracker.settings', JSON.stringify({ theme: settings.theme, mode: settings.mode, density: settings.density }));
        }
        catch {
    ...
    const handleReorder = useCallback((id, before_id, after_id) => {
        repositionTask.mutate({ id, before_id, after_id });
    }, [repositionTask]);
    const handleStatus = useCallback((id, status) => changeStatus.mutate({ id, status }), [changeStatus]);
    ...
    return (_jsxs("div", { className: styles.shell, children: [_jsx(Sidebar, { pages: pages, view: view, onNavigate: (next) => {
                    ...
                    input: { body, occurred_on, status },
                                ...
                                handleCreate(title);
                                setQuickOpen(false);
                            } }), _jsxs("p", { className: styles.quickHint, children: ["Adds to ", currentPage?.name ?? pages[0]?.name ?? 'your first page', " \u00B7 Esc to close"] })] }) }))] }));
}
