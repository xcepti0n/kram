...
describe('rangeFor and zoom', () => {
    ...
    it('keeps the anchor date at the same fraction when zooming', () => {
        ...
        const fromAnchor = (new Date(anchor).getTime() - new Date(zoomed.from).getTime()) / 86_400_000;
...
describe('buildSegments', () => {
    ...
    it('falls back to a single todo segment with no events at all', () => {
        const segments = buildSegments({ created_on: '2026-08-12', completed_on: null, status_events: [] }, TODAY);
        expect(segments).toHaveLength(1);
