...
export function TaskList({
  ...
  const ordered = useMemo(() => {
    ...
    const out = localOrder.map((id) => byId.get(id)).filter((t): t is Task => Boolean(t));
    // Anything new since the drag (created elsewhere) goes to the end.
    for (const task of tasks) if (!localOrder.includes(task.id)) out.push(task);
    return out;
  }, [tasks, localOrder]);
  ...
  return (
    <div className={styles.root}>
      {ordered.length === 0 ? (
        <p className={styles.empty}>Nothing here yet. Add the first task below.</p>
      ) : (
        <DndContext
          sensors={sensors}
      ...
      <TaskComposer onCreate={onCreate} placeholder={composerPlaceholder} />
    </div>
  );
}
